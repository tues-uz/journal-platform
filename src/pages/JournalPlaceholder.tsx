...
import { FileText, Users, Tag, ArrowLeft } from "lucide-react";
  ...
  return (
    <PublicJournalLayout>
      <div className="flex flex-col items-center justify-center py-12 text-center">
        <PublicBreadcrumb items={[{ label: title }]} />
        <div className="w-16 h-16 rounded-2xl bg-gray-100 flex items-center justify-center mb-4">
          <Icon className="h-8 w-8 text-foreground/50" />
        </div>
        <h1 className="text-2xl font-bold text-foreground mb-2">{title}</h1>
        <p className="text-foreground/60 mb-6 max-w-sm">This section is coming soon.</p>
        <Link to={routes.home}>
